...
struct cgm_packet_t *hub_iommu_get_from_queue(){
	...
	if(hub_iommu->next_queue == hub_iommu->Rx_queue_top) //0
	{
		hub_iommu->next_queue = hub_iommu->Rx_queue_top; // 1
	}
	else if(hub_iommu->next_queue == hub_iommu->Rx_queue_bottom)
	{

	}
	else if(hub_iommu->next_queue == hub_iommu->Rx_queue_bottom)
	{
		hub_iommu->next_queue = hub_iommu->Rx_queue_top;
	}
	else
	{
		fatal("hub_iommu_get_from_queue() pointers arn't working");
	}
