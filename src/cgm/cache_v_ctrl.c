	...
	else if(cache_status == 0 || *state_ptr == 0)
	{
		...
		else if(mshr_status == 1)
		{
			...
			CGM_DEBUG(GPU_cache_debug_file, "%s access_id %llu cycle %llu %s as %s\n",
					cache->name, access_id, P_TIME, gpu_l2_caches[cgm_cache_map(cache->id)].name, str_map_value(&cgm_mem_access_strn_map, message_packet->access_type));


			CGM_DEBUG(protocol_debug_file, "%s Access_id %llu cycle %llu %s miss SEND %s %s\n",
					cache->name, access_id, P_TIME, cache->name, gpu_l2_caches[cgm_cache_map(cache->id)].name, str_map_value(&cgm_mem_access_strn_map, message_packet->access_type));
...
void gpu_v_cache_ctrl(void){
	...
	while(1)
	{
		...
		{
			gpu_v_cache_access_load(&(gpu_v_caches[my_pid]), message_packet);
		}
		else if (access_type == cgm_access_store || access_type == cgm_access_nc_store)
		{
			gpu_v_cache_access_store(&(gpu_v_caches[my_pid]), message_packet);
		}
		else if (access_type == cgm_access_retry)
		{
			gpu_v_cache_access_retry(&(gpu_v_caches[my_pid]), message_packet);
		}
		else if (access_type == cgm_access_puts)
		{
			gpu_v_cache_access_puts(&(gpu_v_caches[my_pid]), message_packet);
		}
		else
		{
