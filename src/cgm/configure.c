...
int cache_finish_create(){
	...
	for(i = 0; i < num_cores ; i++ )
	{
		...
		for(l = 0; l < 2; l ++)
		{
			l2_caches[i].share_mask++;

			if(l < (2 - 1))
				l2_caches[i].share_mask = l2_caches[i].share_mask << 1;
		}
		...
		else if(cgm_cache_protocol == cgm_protocol_mesi)
		{
			...
			l2_caches[i].l2_downgrade_nack = cgm_mesi_l2_downgrade_nack;
			l2_caches[i].l2_getx_fwd = cgm_mesi_l2_getx_fwd;
			l2_caches[i].l2_getx_fwd_nack = cgm_mesi_l2_getx_fwd_nack;
		...
		else if(cgm_cache_protocol == cgm_protocol_mesi)
		{
			...
			//l3_caches[i].l3_upgrade_ack = cgm_mesi_l3_upgrade_ack;
	...
	for(i = 0 ; i < gpu_group_cache_num; i++)
	{
		...
		for(l = 0; l < num_cus; l++)
		{
			gpu_l2_caches[i].share_mask++;

			if(l < (num_cus - 1))
				gpu_l2_caches[i].share_mask = gpu_l2_caches[i].share_mask << 1;
		}
