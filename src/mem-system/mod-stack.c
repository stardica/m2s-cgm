...
struct mod_stack_t *mod_stack_create(long long id, struct mod_t *mod, unsigned int addr, int ret_event, struct mod_stack_t *ret_stack)
{
...
void mod_stack_wakeup_stack(struct mod_stack_t *master_stack)
{
	...
	/* Debug */
	mem_debug("  %lld %lld 0x%x wake up accesses:", esim_time, master_stack->id, master_stack->addr);
