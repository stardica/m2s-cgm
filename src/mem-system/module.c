...
struct mod_t *mod_create(char *name, enum mod_kind_t kind, int num_ports, int block_size, int latency)
{
	struct mod_t *mod;
		...
		struct mod_client_info_t *client_info)
{
	...
	/* Initialize */
	stack->witness_ptr = witness_ptr; //= NULL
	stack->event_queue = event_queue; //core->event_queue
	stack->event_queue_item = event_queue_item; //store
	stack->client_info = client_info; // client_info
	...
	if (mod->kind == mod_kind_cache || mod->kind == mod_kind_main_memory)
	{
		...
		else if (access_kind == mod_access_nc_store)
		{
			...
			//NonCachedStores++;
			//printf("entered non cache store\n");
			//fflush(stdout);
...
void mod_access_start(struct mod_t *mod, struct mod_stack_t *stack, enum mod_access_kind_t access_kind)
{
	int index;
 ...
 */
struct mod_stack_t *mod_in_flight_write(struct mod_t *mod, struct mod_stack_t *older_than_stack)
{
	struct mod_stack_t *stack;
 ...
 * return NULL. */
struct mod_stack_t *mod_can_coalesce(struct mod_t *mod, enum mod_access_kind_t access_kind, unsigned int addr, struct mod_stack_t *older_than_stack)
{
	struct mod_stack_t *stack;
	...
	tail = older_than_stack ? older_than_stack->access_list_prev : mod->access_list_tail;
...
void mod_coalesce(struct mod_t *mod, struct mod_stack_t *master_stack, struct mod_stack_t *stack)
{
	/* Debug */
	mem_debug("  %lld %lld 0x%x %s coalesce with %lld\n", esim_time, stack->id, stack->addr, mod->name, master_stack->id);
