...
void mod_handler_nmoesi_store(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_STORE)
	{
		...
		mem_trace("mem.new_access name=\"A-%lld\" type=\"store\" " "state=\"%s:store\" addr=0x%x\n", stack->id, mod->name, stack->addr);
...
void mod_handler_nmoesi_nc_store(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_NC_STORE_LOCK)
	{
		...
		/* Call find and lock */
		new_stack = mod_stack_create(stack->id, mod, stack->addr, EV_MOD_NMOESI_NC_STORE_WRITEBACK, stack);
		new_stack->blocking = 1;
	...
	if (event == EV_MOD_NMOESI_NC_STORE_ACTION)
	{
		...
		{
			new_stack = mod_stack_create(stack->id, mod, stack->tag, EV_MOD_NMOESI_NC_STORE_MISS, stack);
...
void mod_handler_nmoesi_prefetch(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_PREFETCH_LOCK)
	{
		...
		/* Call find and lock */
		new_stack = mod_stack_create(stack->id, mod, stack->addr, EV_MOD_NMOESI_PREFETCH_ACTION, stack);
...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_ACTION)
	{
		...
		{
			stack->eviction = 1;
			new_stack = mod_stack_create(stack->id, mod, 0, EV_MOD_NMOESI_FIND_AND_LOCK_FINISH, stack);
			new_stack->set = stack->set;
			new_stack->way = stack->way;
			esim_schedule_event(EV_MOD_NMOESI_EVICT, new_stack, 0);
			return;
...
void mod_handler_nmoesi_evict(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_EVICT)
	{
		...
		assert(stack->state || !dir_entry_group_shared_or_owned(mod->dir, stack->set, stack->way));
		mem_debug("  %lld %lld 0x%x %s evict (set=%d, way=%d, state=%s)\n", esim_time, stack->id, stack->tag, mod->name, stack->set, stack->way, str_map_value(&cache_block_state_map, stack->state));
	...
	if (event == EV_MOD_NMOESI_EVICT_ACTION)
	{
		...
		if (stack->state == cache_block_modified || stack->state == cache_block_owned || stack->state == cache_block_noncoherent)
		{
		...
		/* Send message */
		stack->msg = net_try_send_ev(mod->low_net, mod->low_net_node, low_node, msg_size, EV_MOD_NMOESI_EVICT_RECEIVE, stack, event, stack);
		return;
	}
	...
	if (event == EV_MOD_NMOESI_EVICT_RECEIVE)
	{
		...
		{
			new_stack = mod_stack_create(stack->id, target_mod, stack->src_tag, EV_MOD_NMOESI_EVICT_PROCESS_NONCOHERENT, stack);
		}
		else 
		{
			new_stack = mod_stack_create(stack->id, target_mod, stack->src_tag, EV_MOD_NMOESI_EVICT_PROCESS, stack);
		}
	...
	if (event == EV_MOD_NMOESI_EVICT_PROCESS)
	{
		...
		else if (stack->reply == reply_ack_data)
		{
			...
			{
				cache_set_block(target_mod->cache, stack->set, stack->way, stack->tag, cache_block_modified);
			}
	...
	if (event == EV_MOD_NMOESI_EVICT_PROCESS_NONCOHERENT)
	{
		...
		if (stack->reply == reply_ack_data)
		{
			...
			else if (stack->state == cache_block_shared || stack->state == cache_block_noncoherent)
			{
				cache_set_block(target_mod->cache, stack->set, stack->way, stack->tag, cache_block_noncoherent);
			}
			else
			{
	...
	if (event == EV_MOD_NMOESI_EVICT_REPLY)
	{
		...
		/* Send message */
		stack->msg = net_try_send_ev(target_mod->high_net, target_mod->high_net_node, mod->low_net_node, 8, EV_MOD_NMOESI_EVICT_REPLY_RECEIVE, stack, event, stack);
		return;
...
void mod_handler_nmoesi_read_request(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST)
	{
		...
		assert(mod_get_low_mod(mod, stack->addr) == target_mod || stack->request_dir == mod_request_down_up);
		assert(mod_get_low_mod(target_mod, stack->addr) == mod || stack->request_dir == mod_request_up_down);
		...
		/* Send message */
		stack->msg = net_try_send_ev(net, src_node, dst_node, 8, EV_MOD_NMOESI_READ_REQUEST_RECEIVE, stack, event, stack);
		return;
	}
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_RECEIVE)
	{
		...
		/* Find and lock */
		new_stack = mod_stack_create(stack->id, target_mod, stack->addr, EV_MOD_NMOESI_READ_REQUEST_ACTION, stack);
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_UPDOWN)
	{
		...
		if (stack->state)
		{
			...
			for (z = 0; z < dir->zsize; z++)
			{
				...
				/* Send read request */
				stack->pending++;
				new_stack = mod_stack_create(stack->id, target_mod, dir_entry_tag, EV_MOD_NMOESI_READ_REQUEST_UPDOWN_FINISH, stack);
