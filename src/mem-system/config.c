...
static void mem_config_read_general(struct config_t *config)
{
	...
	if (!IN_RANGE(mem_frequency, 1, ESIM_MAX_FREQUENCY))
		fatal("%s: invalid value for 'Frequency'.\n%s", mem_config_file_name, mem_err_config_note);
...
static void mem_config_insert_module_in_network(struct config_t *config, struct mod_t *mod, char *net_name, char *net_node_name, struct net_t **net_ptr, struct net_node_t **net_node_ptr)
{
	struct net_t *net;
	...
	*net_node_ptr = NULL;
	...
	{
		fatal("%s: network %s: minimum input buffer size is %d for cache '%s'.\n%s", mem_config_file_name, net->name, mod->block_size + 8, mod->name, mem_err_config_note);
	}
	...
	node = net_add_end_node(net, def_input_buffer_size, def_output_buffer_size, mod->name, mod);
...
static struct mod_t *mem_config_read_cache(struct config_t *config, char *section)
{
	...
	/* Cache parameters */
	snprintf(buf, sizeof buf, "CacheGeometry %s", config_read_string(config, section, "Geometry", ""));
	...
	if (enable_prefetcher)
	{
		...
		if (prefetcher_ghb_size < 1 || prefetcher_it_size < 1 || prefetcher_type == prefetcher_type_invalid || prefetcher_lookup_depth < 2 || prefetcher_lookup_depth > PREFETCHER_LOOKUP_DEPTH_MAX)
		{
	...
	mem_config_insert_module_in_network(config, mod, net_name, net_node_name, &net, &net_node);
	mod->high_net = net;
	...
	char *section)
{
	...
	/* Create module */
	mod = mod_create(mod_name, mod_kind_main_memory, num_ports, block_size, latency);
	...
	struct mod_t *mod, char *section)
{
	...
	if (!strcasecmp(token, "BOUNDS"))
	{
		...
		if (err)
			fatal("%s: %s: invalid value '%s' in 'AddressRange'", mem_config_file_name, mod->name, token);
	...
	int block_size, int level)
{
	...
	if (level > MEM_SYSTEM_MAX_LEVELS)
		fatal("%s: %s: too many cache levels.\n%s%s", mem_config_file_name, mod->name, err_mem_levels, mem_err_config_note);

	/* Check block size */
	if (mod->block_size < block_size)
		fatal("%s: %s: decreasing block size.\n%s%s", mem_config_file_name, mod->name, err_mem_block_size, mem_err_config_note);
...
static void mem_config_read_low_modules(struct config_t *config)
{
	...
	for (i = 0; i < list_count(mem_system->mod_list); i++)
	{
		...
		if (!*low_mod_name_list)
			fatal("%s: [ %s ]: missing or invalid value for 'LowModules'.\n%s", mem_config_file_name, buf, mem_err_config_note);

		/* For each element in the list */
		low_mod_name_list = xstrdup(low_mod_name_list);
		delim = ", ";
		for (low_mod_name = strtok(low_mod_name_list, delim); low_mod_name; low_mod_name = strtok(NULL, delim))
		{
...
static void mem_config_create_switches(struct config_t *config)
{
	...
	for (i = 0; i < list_count(mem_system->net_list); i++)
	{
		...
		net_switch = net_add_switch(net, def_input_buffer_size, def_output_buffer_size, def_bandwidth, net_switch_name);
		...
		for (j = 0; j < list_count(net->node_list); j++)
		{
			...
			{
				net_add_bidirectional_link(net, net_node, net_switch, def_bandwidth, def_output_buffer_size,def_input_buffer_size, 1);
	...
	for (i = 0; i < list_count(mem_system->mod_list); i++)
	{
			...
			linked_list_next(mod->low_mod_list))
		{
			...
			if (mod->low_net != low_mod->high_net)
				fatal("%s: %s: low node '%s' is not in the same network.\n%s", mem_config_file_name, mod->name, low_mod->name, mem_err_config_note);

			/* Check that there is a route */
			entry = net_routing_table_lookup(mod->low_net->routing_table, mod->low_net_node, low_mod->high_net_node);

			if (!entry->output_buffer)
				fatal("%s: %s: network does not connect '%s' with '%s'.\n%s", mem_config_file_name, mod->low_net->name, mod->name, low_mod->name, err_mem_connect);
		}
			...
			linked_list_next(mod->high_mod_list))
		{
			...
			if (mod->high_net != high_mod->low_net)
				fatal("%s: %s: high node '%s' is not in the same network.\n%s", mem_config_file_name, mod->name, high_mod->name, mem_err_config_note);

			/* Check that there is a route */
			entry = net_routing_table_lookup(mod->high_net->routing_table, mod->high_net_node, high_mod->low_net_node);
			if (!entry->output_buffer)
				fatal("%s: %s: network does not connect '%s' with '%s'.\n%s", mem_config_file_name, mod->high_net->name, mod->name, high_mod->name, err_mem_connect);
		}
...
static void mem_config_check_disjoint(struct arch_t *arch, void *user_data)
{
	...
	LINKED_LIST_FOR_EACH(arch->mem_entry_mod_list)
	{
		...
		if (mod_arch != arch)
			fatal("%s: architectures '%s' and '%s' share memory modules.\n%s", mem_config_file_name, arch->name, mod_arch->name, err_mem_disjoint);
	}
}
...
static void mem_config_calculate_sub_block_sizes(void)
{
	...
	for (i = 0; i < list_count(mem_system->mod_list); i++)
	{
		...
		for (linked_list_head(mod->high_mod_list); !linked_list_is_end(mod->high_mod_list); linked_list_next(mod->high_mod_list))
		{
			high_mod = linked_list_get(mod->high_mod_list);
...
static void mem_config_calculate_mod_levels_arch(struct arch_t *arch, void *user_data)
{
	struct mod_t *mod;
...
static void mem_config_read_commands(struct config_t *config)
{
	...
	EV_MEM_SYSTEM_COMMAND = esim_register_event_with_name(mem_system_command_handler, mem_domain_index, "mem_system_command");
	EV_MEM_SYSTEM_END_COMMAND = esim_register_event_with_name(mem_system_end_command_handler, mem_domain_index, "mem_system_end_command");
