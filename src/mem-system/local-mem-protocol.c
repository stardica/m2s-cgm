...
void mod_handler_local_mem_load(int event, void *data)
{
	...
	if (event == EV_MOD_LOCAL_MEM_LOAD_LOCK)
	{
		...
		new_stack = mod_stack_create(stack->id, mod, stack->addr, EV_MOD_LOCAL_MEM_LOAD_FINISH, stack);
		new_stack->read = 1;
...
void mod_handler_local_mem_store(int event, void *data)
{
	...
	if (event == EV_MOD_LOCAL_MEM_STORE_LOCK)
	{
		...
		/* Call find and lock */
		new_stack = mod_stack_create(stack->id, mod, stack->addr, EV_MOD_LOCAL_MEM_STORE_FINISH, stack);
		new_stack->read = 0;
