...
void si_isa_init()
{
	...
	//star >> typedef void (*si_isa_inst_func_t)(struct si_work_item_t *work_item, struct si_inst_t *inst);
	//star >> si_isa_inst_func_t *si_isa_inst_func;

	/* Initialize */
	//star >> xcalloc will create array addressable memory.
	//so what we really have is...		si_isa_inst_func[SI_INST_COUNT];
	si_isa_inst_func = xcalloc(SI_INST_COUNT, sizeof(si_isa_inst_func_t));


	//	DEFINST(S_ADD_U32, " s_add_u32     %SDST, %SSRC0, %SSRC1", SOP2, 0, 4, 0)
	//	extern void si_isa_S_ADD_U32_impl(struct si_work_item_t *work_item, struct si_inst_t *inst);

	//	si_isa_inst_func[SI_INST_S_ADD_U32] = si_isa_S_ADD_U32_impl;
...
#include <arch/si/asm/asm.dat>
#undef DEFINST

	/* Repository of deferred tasks */
	si_isa_write_task_repos = repos_create(sizeof(struct si_isa_write_task_t), "gpu_isa_write_task_repos");
}
