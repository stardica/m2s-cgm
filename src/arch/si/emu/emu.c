...
#include <arch/si/emu/opengl-bin-file.h>
...
int SIEmuRun(Emu *self)
{
	...
	if (!list_count(emu->running_work_groups) && list_count(emu->waiting_work_groups))
	{
...
void si_emu_init(void)
{
	...
	if (*si_emu_report_file_name)
	{
		...
		if (!si_emu_report_file)
			fatal("%s: cannot open report for Southern Islands emulator", si_emu_report_file_name);
	}
