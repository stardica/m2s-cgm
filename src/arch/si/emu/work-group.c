...
struct si_work_group_t *si_work_group_create(unsigned int work_group_id, struct si_ndrange_t *ndrange)
{
	...
	unsigned int work_items_per_group = ndrange->local_size3[0] * ndrange->local_size3[1] * ndrange->local_size3[2];
	assert(work_items_per_group > 0);

	/* Number of wavefronts in work-group */
	unsigned int wavefronts_per_group = (work_items_per_group + (si_emu_wavefront_size - 1)) / si_emu_wavefront_size;
	...
	/* Allocate pointers for work-items (will actually be created when wavefronts are created) */
	work_group->work_items = xcalloc(si_emu_wavefront_size * wavefronts_per_group, sizeof(void *));
	...
	{
		work_group->wavefronts[wavefront_id] = si_wavefront_create(work_group->id * wavefronts_per_group + wavefront_id, work_group);
		...
		SI_FOREACH_WORK_ITEM_IN_WAVEFRONT(wavefront, work_item_id)
		{
			...
			work_group->work_items[wavefront_offset+work_item_id] = wavefront->work_items[work_item_id];
			work_group->work_items[wavefront_offset+work_item_id]-> work_group = work_group;
		}
	}

	/* Initialize work-group and work-item metadata */
	work_group->id_3d[0] = work_group_id % ndrange->group_count3[0];
	work_group->id_3d[1] = (work_group_id / ndrange->group_count3[0]) % ndrange->group_count3[1];
	work_group->id_3d[2] = work_group_id / (ndrange->group_count3[0] * ndrange->group_count3[1]);
	work_group->id = work_group_id;

	/* Number of work-items in work-group */
	work_group->work_item_count = ndrange->local_size3[2] * ndrange->local_size3[1] * ndrange->local_size3[0];
	...
	for (lidz = 0; lidz < ndrange->local_size3[2]; lidz++)
	{
		for (lidy = 0; lidy < ndrange->local_size3[1]; lidy++)
		{
			for (lidx = 0; lidx < ndrange->local_size3[0]; lidx++)
			{
				...
				{
					wavefront->sreg[SI_EXEC + 1].as_uint |= 1 << work_item->id_in_wavefront;
				}
				else 
				{
					fatal("%s: invalid work-item id (%d)", __FUNCTION__, work_item->id_in_wavefront);
				}
			}
		}
	}
	...
	SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id)
	{
		...
		wavefront->sreg[ndrange->wg_id_sgpr].as_int = wavefront->work_group->id_3d[0];
		wavefront->sreg[ndrange->wg_id_sgpr + 1].as_int = wavefront->work_group->id_3d[1];
		wavefront->sreg[ndrange->wg_id_sgpr + 2].as_int = wavefront->work_group->id_3d[2];
		...
		for (i = 0; i < user_element_count; i++)
		{
			if (user_elements[i].dataClass == IMM_CONST_BUFFER)
			{
				...
				si_wavefront_init_sreg_with_cb(wavefront, user_elements[i].startUserReg, user_elements[i].userRegCount, user_elements[i].apiSlot);
			}
			else if (user_elements[i].dataClass == IMM_UAV)
			{
				/* Store UAV pointer in sregs */
				si_wavefront_init_sreg_with_uav(wavefront, user_elements[i].startUserReg, user_elements[i].userRegCount, user_elements[i].apiSlot);
			}
			else if (user_elements[i].dataClass == PTR_CONST_BUFFER_TABLE)
			{
				/* Store CB table in sregs */
				si_wavefront_init_sreg_with_cb_table(wavefront, user_elements[i].startUserReg, user_elements[i].userRegCount);
			}
			else if (user_elements[i].dataClass == PTR_UAV_TABLE)
			{
				/* Store UAV table in sregs */
				si_wavefront_init_sreg_with_uav_table(wavefront, user_elements[i].startUserReg, user_elements[i].userRegCount);
			}
			else if (user_elements[i].dataClass == PTR_VERTEX_BUFFER_TABLE)
			{
				/* Store VB table in sregs */
				si_wavefront_init_sreg_with_vertex_buffer_table(wavefront, user_elements[i].startUserReg, user_elements[i].userRegCount);
			}
			else if (user_elements[i].dataClass == SUB_PTR_FETCH_SHADER)
			{
				/* Store Fetch Shader pointer in sregs */
				si_wavefront_init_sreg_with_fetch_shader(wavefront, user_elements[i].startUserReg, user_elements[i].userRegCount);
			}
			...
			{
				fatal("%s: PTR_INTERNAL_GLOBAL_TABLE not supported", __FUNCTION__);
			}
			else
			{
				fatal("%s: Unimplemented User Element: dataClass:%d", __FUNCTION__, user_elements[i].dataClass);
			}
		}
	}
