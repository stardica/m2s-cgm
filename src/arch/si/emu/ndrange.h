...
#include <arch/si/asm/bin-file.h>
...
struct si_ndrange_t
{
	...
	struct si_ndrange_table_entry_t const_buf_table_entries[SI_EMU_MAX_NUM_CONST_BUFS];
	unsigned int resource_table;
	struct si_ndrange_table_entry_t resource_table_entries[SI_EMU_MAX_NUM_RESOURCES];
	...
	struct si_ndrange_table_entry_t vertex_buffer_table_entries[SI_EMU_MAX_NUM_VERTEX_BUFFERS];
...
void si_ndrange_setup_size(struct si_ndrange_t *ndrange, unsigned int *global_size, unsigned int *local_size, int work_dim);
void si_ndrange_setup_fs_mem(struct si_ndrange_t *ndrange, void *buf, int size, unsigned int pc);
void si_ndrange_setup_inst_mem(struct si_ndrange_t *ndrange, void *buf, int size, unsigned int pc);

/* Access constant buffers */
void si_ndrange_const_buf_write(struct si_ndrange_t *ndrange, int const_buf_num, int offset, void *pvalue, unsigned int size);
...
void si_ndrange_insert_buffer_into_uav_table(struct si_ndrange_t *ndrange, struct si_buffer_desc_t *buf_desc, unsigned int uav);
void si_ndrange_insert_buffer_into_vertex_buffer_table(struct si_ndrange_t *ndrange, struct si_buffer_desc_t *buf_desc, unsigned int vertex_buffer);
void si_ndrange_insert_buffer_into_const_buf_table(struct si_ndrange_t *ndrange, struct si_buffer_desc_t *buf_desc, unsigned int const_buf_num);
void si_ndrange_insert_image_into_uav_table(struct si_ndrange_t *ndrange, struct si_image_desc_t *image_desc, unsigned int uav);
