...
struct si_wavefront_t *si_wavefront_create(int wavefront_id, struct si_work_group_t *work_group)
{
...
void si_wavefront_execute(struct si_wavefront_t *wavefront)
{
	...
	wavefront->inst_size = si_inst_decode(ndrange->inst_buffer + wavefront->pc, &wavefront->inst, 0);
	...
	case SI_FMT_SOP1:
	{
		...
		{
			si_inst_dump(inst, wavefront->inst_size, wavefront->pc, ndrange->inst_buffer + wavefront->pc, inst_dump, MAX_INST_STR_SIZE);
			si_isa_debug("\n%s", inst_dump);
		}
		...
		//star >> this is where the instruction function is run.
	...
	case SI_FMT_SOPP:
	{
		...
		/* Stats */
		if (wavefront->inst.micro_inst.sopp.op > 1 && wavefront->inst.micro_inst.sopp.op < 10)
		{
	...
	case SI_FMT_DS:
	{
		...
		/* Record access type */
		if ((inst->info->opcode >= 13 && inst->info->opcode < 16) || (inst->info->opcode >= 30 && inst->info->opcode < 32) || (inst->info->opcode >= 77 && inst->info->opcode < 80))
		{
			wavefront->lds_write = 1;
		}
		else if (
			(inst->info->opcode >= 54 && inst->info->opcode < 61) || (inst->info->opcode >= 118 && inst->info->opcode < 120))
		{
	...
	case SI_FMT_MUBUF:
	{
		...
		/* Record access type */
		if ((inst->info->opcode >= 0 && inst->info->opcode < 4) || (inst->info->opcode >= 8 && inst->info->opcode < 15))
		{
			wavefront->vector_mem_read = 1;
		}
		else if ((inst->info->opcode >= 4 && inst->info->opcode < 8) || (inst->info->opcode >= 24 && inst->info->opcode < 30))
		{
			wavefront->vector_mem_write = 1;
		}
		else 
	...
	default:
	{
		fatal("%s: instruction type not implemented (%d)", __FUNCTION__, inst->info->fmt);
		break;
	}
...
void si_wavefront_init_sreg_with_cb(struct si_wavefront_t *wavefront, int first_reg, int num_regs, int const_buf_num)
{
	...
	buf_desc_addr = ndrange->const_buf_table + const_buf_num*SI_EMU_CONST_BUF_TABLE_ENTRY_SIZE;
...
void si_wavefront_init_sreg_with_cb_table(struct si_wavefront_t *wavefront, int first_reg, int num_regs)
{
	struct si_ndrange_t *ndrange = wavefront->work_group->ndrange;
 ...
 * (requires 4 consecutive registers to store the 128-bit structure) */
void si_wavefront_init_sreg_with_uav(struct si_wavefront_t *wavefront, int first_reg, int num_regs, int uav)
{
	struct si_buffer_desc_t buf_desc;
	struct si_ndrange_t *ndrange = wavefront->work_group->ndrange;
...
void si_wavefront_init_sreg_with_uav_table(struct si_wavefront_t *wavefront, int first_reg, int num_regs)
{
	struct si_ndrange_t *ndrange = wavefront->work_group->ndrange;
	struct si_mem_ptr_t mem_ptr;
...
void si_wavefront_init_sreg_with_vertex_buffer_table(struct si_wavefront_t *wavefront, int first_reg, int num_regs)
{
	struct si_ndrange_t *ndrange = wavefront->work_group->ndrange;
	struct si_mem_ptr_t mem_ptr;
...
void si_wavefront_init_sreg_with_fetch_shader(struct si_wavefront_t *wavefront, int first_reg, int num_regs)
{
	struct si_ndrange_t *ndrange = wavefront->work_group->ndrange;
	struct si_mem_ptr_t mem_ptr;
