...
struct si_wavefront_pool_t *si_wavefront_pool_create()
{
	...
	wavefront_pool->entries = xcalloc(si_gpu_max_wavefronts_per_wavefront_pool, sizeof(struct si_wavefront_pool_entry_t*));
...
void si_wavefront_pool_entry_clear(struct si_wavefront_pool_entry_t *wavefront_pool_entry)
{
...
void si_wavefront_pool_unmap_wavefronts(struct si_wavefront_pool_t *wavefront_pool, struct si_work_group_t *work_group)
{
	...
	for (i = 0; i < work_group->wavefront_count; i++) 
	{
		...
		assert(wavefront_pool->entries[wf_id_in_wfp]->wavefront->id == wavefront->id);
