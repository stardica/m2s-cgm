...
#include <arch/si/timing/branch-unit.h>
...
void si_branch_unit_complete(struct si_branch_unit_t *branch_unit)
{
	...
	assert(list_entries <= si_gpu_branch_unit_write_latency * si_gpu_branch_unit_width);
...
void si_branch_unit_write(struct si_branch_unit_t *branch_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		/* Sanity check the write buffer */
		assert(list_count(branch_unit->write_buffer) <= si_gpu_branch_unit_write_buffer_size);

		/* Stall if the write buffer is full. */
		if (list_count(branch_unit->write_buffer) == si_gpu_branch_unit_write_buffer_size)
		{
			si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld "
				"stg=\"s\"\n", uop->id_in_compute_unit, 
				branch_unit->compute_unit->id, 
		...
		uop->write_ready = asTiming(si_gpu)->cycle + si_gpu_branch_unit_write_latency;
...
void si_branch_unit_execute(struct si_branch_unit_t *branch_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		/* Sanity check the exec buffer */
		assert(list_count(branch_unit->exec_buffer) <= si_gpu_branch_unit_exec_buffer_size);

		/* Stall if the exec buffer is full. */
		if (list_count(branch_unit->exec_buffer) == si_gpu_branch_unit_exec_buffer_size)
		{
			si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld "
				"stg=\"s\"\n", uop->id_in_compute_unit, 
				branch_unit->compute_unit->id, 
		...
		/* Branch */
		uop->execute_ready = asTiming(si_gpu)->cycle + si_gpu_branch_unit_exec_latency;
...
void si_branch_unit_read(struct si_branch_unit_t *branch_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		/* Sanity check the read buffer */
		assert(list_count(branch_unit->read_buffer) <= si_gpu_branch_unit_read_buffer_size);
...
void si_branch_unit_decode(struct si_branch_unit_t *branch_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		assert(list_count(branch_unit->decode_buffer) <= si_gpu_branch_unit_decode_buffer_size);

		/* Stall if the decode buffer is full. */
		if (list_count(branch_unit->decode_buffer) == si_gpu_branch_unit_decode_buffer_size)
		{
			si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld "
				"stg=\"s\"\n", uop->id_in_compute_unit, 
				branch_unit->compute_unit->id, 
		...
		uop->decode_ready = asTiming(si_gpu)->cycle + si_gpu_branch_unit_decode_latency;
