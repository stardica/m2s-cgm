...
#include <arch/si/timing/simd-unit.h>
...
struct si_compute_unit_t *si_compute_unit_create()
{
	...
	compute_unit->wavefront_pools = xcalloc(compute_unit->num_wavefront_pools, sizeof(struct si_wavefront_pool_t*));
	compute_unit->fetch_buffers = xcalloc(compute_unit->num_wavefront_pools, sizeof(struct list_t*));
	compute_unit->simd_units = xcalloc(compute_unit->num_wavefront_pools, sizeof(struct si_simd_t*));
	...
	//compute_unit->vector_mem_unit.id = compute_unit->id;
	...
	for (i = 0; i < compute_unit->num_wavefront_pools; i++) 
	{
		...
		compute_unit->simd_units[i]->wavefront_pool = compute_unit->wavefront_pools[i];
		...
		compute_unit->simd_units[i]->subwavefront_pool = xcalloc(1, sizeof(struct si_subwavefront_pool_t));

		compute_unit->simd_units[i]->compute_unit = compute_unit;
		compute_unit->simd_units[i]->wkg_util = xcalloc(1, sizeof(struct si_util_t));
		compute_unit->simd_units[i]->wvf_util = xcalloc(1, sizeof(struct si_util_t));
		compute_unit->simd_units[i]->rdy_util = xcalloc(1, sizeof(struct si_util_t));
		compute_unit->simd_units[i]->occ_util = xcalloc(1, sizeof(struct si_util_t));
		compute_unit->simd_units[i]->wki_util = xcalloc(1, sizeof(struct si_util_t));
		compute_unit->simd_units[i]->act_util = xcalloc(1, sizeof(struct si_util_t));
		compute_unit->simd_units[i]->tot_util = xcalloc(1, sizeof(struct si_util_t));
	}

	compute_unit->work_groups = xcalloc(si_gpu_max_work_groups_per_wavefront_pool * si_gpu_num_wavefront_pools, sizeof(void *));
...
void si_compute_unit_map_work_group(struct si_compute_unit_t *compute_unit, struct si_work_group_t *work_group)
{
	...
	while (work_group->id_in_compute_unit < si_gpu->work_groups_per_compute_unit && compute_unit->work_groups[work_group->id_in_compute_unit])
	{
	...
	SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(work_group, wavefront_id)
	{
		...
		wavefront->id_in_compute_unit = work_group->id_in_compute_unit * work_group->wavefront_count + wavefront_id;
	}
...
void si_compute_unit_unmap_work_group(struct si_compute_unit_t *compute_unit, struct si_work_group_t *work_group)
{
	...
	assert(list_index_of(si_emu->running_work_groups, (void*)work_group_id) >= 0);
	list_remove(si_emu->running_work_groups, (void*)work_group_id);

	if (!list_count(si_emu->running_work_groups) && !list_count(si_emu->waiting_work_groups))
	{
...
void si_compute_unit_fetch(struct si_compute_unit_t *compute_unit, int active_fb)
{
	...
	for (i = 0; i < si_gpu_max_wavefronts_per_wavefront_pool; i++)
	{
		...
		assert(wavefront->wavefront_pool_entry == compute_unit->wavefront_pools[active_fb]->entries[i]);
		...
		{
			//printf("waiting id %d cycle %llu\n", wavefront->id, P_TIME);

			if (!wavefront->wavefront_pool_entry->lgkm_cnt && !wavefront->wavefront_pool_entry->vm_cnt)
			{
		...
		assert(list_count(compute_unit->fetch_buffers[active_fb]) <= si_gpu_fe_fetch_buffer_size);
		if (list_count(compute_unit->fetch_buffers[active_fb]) == si_gpu_fe_fetch_buffer_size)
		{
			continue;
		}
...
void si_compute_unit_issue_oldest(struct si_compute_unit_t *compute_unit, int active_fb)
{
	...
	enum gpu_issue_state_t branch_gpu_state = idle;
	enum gpu_issue_state_t scalar_gpu_state = idle;
	enum gpu_issue_state_t vector_gpu_state = idle;
	enum gpu_issue_state_t simd_gpu_state = idle;
	enum gpu_issue_state_t lds_gpu_state = idle;
	...
	for (issued_insts = 0; issued_insts < si_gpu_fe_max_inst_issued_per_type; issued_insts++)
	{
		...
		for (i = 0; i < list_entries; i++)
		{
			...
			if (oldest_uop && list_count(compute_unit->branch_unit.issue_buffer) < si_gpu_branch_unit_issue_buffer_size)
			{

				branch_gpu_state = busy;
	...
	for (issued_insts = 0; issued_insts < si_gpu_fe_max_inst_issued_per_type;issued_insts++)
	{
		for (i = 0; i < list_entries; i++)
		{
			...
			if (uop->inst.info->fmt == SI_FMT_SOPP && uop->inst.micro_inst.sopp.op > 1 && uop->inst.micro_inst.sopp.op < 10)
			{
				list_index++;
				continue;
			}
			...
			if (oldest_uop && list_count(compute_unit->scalar_unit.issue_buffer) < si_gpu_scalar_unit_issue_buffer_size)
			{

				scalar_gpu_state = busy;
	...
	for (issued_insts = 0; issued_insts < si_gpu_fe_max_inst_issued_per_type; issued_insts++)
	{
		...
		for (i = 0; i < list_entries; i++)
		{
			...
			if(vector_gpu_state != busy)
				vector_gpu_state = stalled;
			...
			if (!oldest_uop || uop->fetch_ready < oldest_uop->fetch_ready)
			{
				/*if(compute_unit->id == 0)
				{
					printf("running list_entries %d cycle %llu\n", list_entries, P_TIME);
					getchar();
				}*/
			...
			if (oldest_uop && list_count(compute_unit->vector_mem_unit.issue_buffer) < si_gpu_vector_mem_issue_buffer_size)
			{
				vector_gpu_state = busy;
			...
			if(vector_gpu_state == stalled)
				assert(list_count(compute_unit->vector_mem_unit.issue_buffer) == si_gpu_vector_mem_issue_buffer_size);
		}
	}
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld stg=\"s\"\n", uop->id_in_compute_unit, compute_unit->id, uop->wavefront->id, uop->id_in_wavefront);
	}
...
void si_compute_unit_update_fetch_visualization(struct si_compute_unit_t *compute_unit, int non_active_fb)
{
	struct si_uop_t *uop;
	int list_entries;
	int i;
...
long long cu_unmapped_cycles = 0;
long long cu_mapped_cycles = 0;
...
void si_compute_unit_run(struct si_compute_unit_t *compute_unit)
{
	...
	cpu_gpu_stats->cu_total_mapped[compute_unit->id] += (1 * (x86_cpu_frequency/si_gpu_frequency));

	/* Fetch buffer chosen to issue this cycle */
	active_fetch_buffer = asTiming(si_gpu)->cycle % compute_unit->num_wavefront_pools;

	assert(active_fetch_buffer >= 0 && active_fetch_buffer < compute_unit->num_wavefront_pools);
