...
void SIGpuMemConfigDefault(Timing *self, struct config_t *config)
{
	...
	for (i = 0; i < (si_gpu_num_compute_units + 3) / 4; i++)
	{
		...
		config_write_string(config, section, "LowModules", "si-l2-0 si-l2-1 si-l2-2 si-l2-3 si-l2-4 si-l2-5");
	}
	...
	config_write_string(config, section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 0");
	...
	config_write_string(config, section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 1");
	...
	config_write_string(config, section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 2");
	...
	config_write_string(config, section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 3");
	...
	config_write_string(config, section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 4");
	...
	config_write_string(config, section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 5");
...
void SIGpuMemConfigParseEntry(Timing *self, struct config_t *config, char *section)
{
	...
	if (separate_present)
	{
		vector_module_name = config_read_string(config, section, "DataModule", NULL);
		scalar_module_name = config_read_string(config, section, "ConstantDataModule", NULL);
	}
	else
	{
