...
void si_scalar_unit_complete(struct si_scalar_unit_t *scalar_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		if (uop->barrier_wait_inst)
		{
			...
			{
				if(!uop->work_group->wavefronts[wavefront_id]->wavefront_pool_entry->wait_for_barrier)
				{
					barrier_complete = 0;
				}
			}
			...
			if (barrier_complete)
			{
				SI_FOREACH_WAVEFRONT_IN_WORK_GROUP(uop->work_group, wavefront_id)
				{
					assert(uop->work_group->wavefronts[wavefront_id]->wavefront_pool_entry->wait_for_barrier);
					uop->work_group->wavefronts[wavefront_id]->wavefront_pool_entry->wait_for_barrier = 0;
				}
			}
		}
		...
		if (uop->wavefront_last_inst)
		{
			...
			if (uop->work_group->wavefronts_completed_timing == uop->work_group->wavefront_count)
			{
			...
			assert(uop->work_group->wavefronts_completed_timing <= uop->work_group->wavefront_count);
			if (uop->work_group->finished_timing)
			{
				assert(uop->work_group->wavefronts_completed_timing == uop->work_group->wavefront_count);

				si_compute_unit_unmap_work_group(scalar_unit->compute_unit,uop->work_group);
			}
		}
...
void si_scalar_unit_write(struct si_scalar_unit_t *scalar_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		if (uop->scalar_mem_read)
		{
			...
			uop->write_ready = asTiming(si_gpu)->cycle + si_gpu_scalar_unit_write_latency;
...
void si_scalar_unit_execute(struct si_scalar_unit_t *scalar_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		assert(list_count(scalar_unit->exec_buffer) <= si_gpu_scalar_unit_exec_buffer_size);

		/* Stall if there is not room in the exec buffer */
		if (list_count(scalar_unit->exec_buffer) == si_gpu_scalar_unit_exec_buffer_size)
		{
			si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld "
				"stg=\"s\"\n", uop->id_in_compute_unit, 
				scalar_unit->compute_unit->id, 
		...
		{
			uop->execute_ready = asTiming(si_gpu)->cycle + si_gpu_scalar_unit_exec_latency;
...
void si_scalar_unit_read(struct si_scalar_unit_t *scalar_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		assert(list_count(scalar_unit->read_buffer) <= si_gpu_scalar_unit_read_buffer_size);
...
void si_scalar_unit_decode(struct si_scalar_unit_t *scalar_unit)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		assert(list_count(scalar_unit->decode_buffer) <= si_gpu_scalar_unit_decode_buffer_size);
		...
		uop->decode_ready = asTiming(si_gpu)->cycle + si_gpu_scalar_unit_decode_latency;
