...
char *si_gpu_driver_version = "4.2";
...
void si_gpu_map_ndrange(struct si_ndrange_t *ndrange)
{
	 ...
	 * wavefront pool */
	si_gpu->work_groups_per_wavefront_pool = si_calc_get_work_groups_per_wavefront_pool(ndrange->local_size, ndrange->num_vgpr_used, ndrange->local_mem_top);
	...
	si_gpu->work_groups_per_compute_unit = si_gpu->work_groups_per_wavefront_pool *  si_gpu_num_wavefront_pools;
...
void si_config_dump(FILE *f)
{
	...
	fprintf(f, "RegisterAllocGranularity = %s\n", str_map_value(&si_gpu_register_alloc_granularity_map, si_gpu_register_alloc_granularity));
	*/
...
void si_gpu_read_config(void)
{
	...
	char *section;
	char *err_note = "\tPlease run 'm2s --si-help' or consult the Multi2Sim Guide for a\n\tdescription of the GPU configuration file format.";
	...
	si_gpu_frequency = config_read_int(gpu_config, section, "Frequency", si_gpu_frequency);
	if (!IN_RANGE(si_gpu_frequency, 1, ESIM_MAX_FREQUENCY))
		fatal("%s: invalid value for 'Frequency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_num_compute_units = config_read_int(gpu_config, section, "NumComputeUnits", si_gpu_num_compute_units);
	...
	si_gpu_num_wavefront_pools = config_read_int(gpu_config, section, "NumWavefrontPools", si_gpu_num_wavefront_pools);
	if (si_gpu_num_wavefront_pools < 1)
		fatal("%s: invalid value for 'NumWavefrontPools'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_max_work_groups_per_wavefront_pool = config_read_int(gpu_config, section, "MaxWorkGroupsPerWavefrontPool", si_gpu_max_work_groups_per_wavefront_pool);
	if (si_gpu_max_work_groups_per_wavefront_pool < 1)
		fatal("%s: invalid value for 'MaxWorkGroupsPerWavefrontPool'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_max_wavefronts_per_wavefront_pool = config_read_int(gpu_config, section, "MaxWavefrontsPerWavefrontPool", si_gpu_max_wavefronts_per_wavefront_pool);
	if (si_gpu_max_wavefronts_per_wavefront_pool < 1)
		fatal("%s: invalid value for 'MaxWavefrontsPerWavefrontPool'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_num_vector_registers = config_read_int(gpu_config, section, "NumVectorRegisters", si_gpu_num_vector_registers);
	if (si_gpu_num_vector_registers < 1)
		fatal("%s: invalid value for 'NumVectorRegisters'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_num_scalar_registers = config_read_int(gpu_config, section, "NumScalarRegisters", si_gpu_num_scalar_registers);
	...
	si_gpu_fe_fetch_latency = config_read_int(gpu_config, section, "FetchLatency", si_gpu_fe_fetch_latency);
	if (si_gpu_fe_fetch_latency < 0)
		fatal("%s: invalid value for 'FetchLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_fe_fetch_width = config_read_int(gpu_config, section, "FetchWidth", si_gpu_fe_fetch_width);
	if (si_gpu_fe_fetch_width < 1)
		fatal("%s: invalid value for 'FetchWidth'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_fe_fetch_buffer_size = config_read_int(gpu_config, section, "FetchBufferSize", si_gpu_fe_fetch_buffer_size);
	if (si_gpu_fe_fetch_buffer_size < 1)
		fatal("%s: invalid value for 'FetchBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_fe_issue_latency = config_read_int(gpu_config, section, "IssueLatency", si_gpu_fe_issue_latency);
	if (si_gpu_fe_issue_latency < 0)
		fatal("%s: invalid value for 'IssueLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_fe_issue_width = config_read_int(gpu_config, section, "IssueWidth", si_gpu_fe_issue_width);
	if (si_gpu_fe_issue_width < 1)
		fatal("%s: invalid value for 'IssueWidth'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_fe_max_inst_issued_per_type = config_read_int(gpu_config, section, "MaxInstIssuedPerType", si_gpu_fe_max_inst_issued_per_type);
	...
	si_gpu_simd_num_simd_lanes = config_read_int(gpu_config, section, "NumSIMDLanes", si_gpu_simd_num_simd_lanes);
	if (si_gpu_simd_num_simd_lanes < 1)
		fatal("%s: invalid value for 'NumSIMDLanes'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_simd_width = config_read_int(gpu_config, section, "Width", si_gpu_simd_width);
	if (si_gpu_simd_width < 1)
		fatal("%s: invalid value for 'Width'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_simd_issue_buffer_size = config_read_int(gpu_config, section, "IssueBufferSize", si_gpu_simd_issue_buffer_size);
	if (si_gpu_simd_issue_buffer_size < 1)
		fatal("%s: invalid value for 'IssueBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_simd_decode_latency = config_read_int(gpu_config, section, "DecodeLatency", si_gpu_simd_decode_latency);
	if (si_gpu_simd_decode_latency < 0)
		fatal("%s: invalid value for 'DecodeLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_simd_decode_buffer_size = config_read_int(gpu_config, section, "DecodeBufferSize", si_gpu_simd_decode_buffer_size);
	...
	si_gpu_simd_exec_latency = config_read_int(gpu_config, section, "ReadExecWriteLatency", si_gpu_simd_exec_latency);
	if (si_gpu_simd_exec_latency < 0)
		fatal("%s: invalid value for 'ReadExecWriteLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_simd_exec_buffer_size = config_read_int(gpu_config, section, "ReadExecWriteBufferSize", si_gpu_simd_exec_buffer_size);
	...
	si_gpu_scalar_unit_width = config_read_int(gpu_config, section, "Width", si_gpu_scalar_unit_width);
	if (si_gpu_scalar_unit_width < 1)
		fatal("%s: invalid value for 'Width'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_issue_buffer_size = config_read_int(gpu_config, section, "IssueBufferSize", si_gpu_scalar_unit_issue_buffer_size);
	if (si_gpu_scalar_unit_issue_buffer_size < 1)
		fatal("%s: invalid value for 'IssueBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_decode_latency = config_read_int(gpu_config, section, "DecodeLatency", si_gpu_scalar_unit_decode_latency);
	if (si_gpu_scalar_unit_decode_latency < 0)
		fatal("%s: invalid value for 'DecodeLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_decode_buffer_size = config_read_int(gpu_config, section, "DecodeBufferSize", si_gpu_scalar_unit_decode_buffer_size);
	if (si_gpu_scalar_unit_decode_buffer_size < 1)
		fatal("%s: invalid value for 'DecodeBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_read_latency = config_read_int(gpu_config, section, "ReadLatency", si_gpu_scalar_unit_read_latency);
	if (si_gpu_scalar_unit_read_latency < 0)
		fatal("%s: invalid value for 'ReadLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_read_buffer_size = config_read_int(gpu_config, section, "ReadBufferSize", si_gpu_scalar_unit_read_buffer_size);
	if (si_gpu_scalar_unit_read_buffer_size < 1)
		fatal("%s: invalid value for 'ReadBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_exec_latency = config_read_int(gpu_config, section, "ALULatency", si_gpu_scalar_unit_exec_latency);
	if (si_gpu_scalar_unit_exec_latency < 0)
		fatal("%s: invalid value for 'ALULatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_exec_buffer_size= config_read_int(gpu_config, section, "ExecBufferSize", si_gpu_scalar_unit_exec_buffer_size);
	if (si_gpu_scalar_unit_exec_buffer_size < 1)
		fatal("%s: invalid value for 'ExecBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_write_latency = config_read_int(gpu_config, section, "WriteLatency", si_gpu_scalar_unit_write_latency);
	if (si_gpu_scalar_unit_write_latency < 0)
		fatal("%s: invalid value for 'WriteLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_scalar_unit_write_buffer_size = config_read_int(gpu_config, section, "WriteBufferSize", si_gpu_scalar_unit_write_buffer_size);
	...
	si_gpu_branch_unit_width = config_read_int(gpu_config, section, "Width", si_gpu_branch_unit_width);
	if (si_gpu_branch_unit_width < 1)
		fatal("%s: invalid value for 'Width'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_issue_buffer_size = config_read_int(gpu_config, section, "IssueBufferSize", si_gpu_branch_unit_issue_buffer_size);
	if (si_gpu_branch_unit_issue_buffer_size < 1)
		fatal("%s: invalid value for 'IssueBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_decode_latency = config_read_int(gpu_config, section, "DecodeLatency", si_gpu_branch_unit_decode_latency);
	if (si_gpu_branch_unit_decode_latency < 0)
		fatal("%s: invalid value for 'DecodeLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_decode_buffer_size = config_read_int(gpu_config, section, "DecodeBufferSize", si_gpu_branch_unit_decode_buffer_size);
	if (si_gpu_branch_unit_decode_buffer_size < 1)
		fatal("%s: invalid value for 'DecodeBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_read_latency = config_read_int(gpu_config, section, "ReadLatency", si_gpu_branch_unit_read_latency);
	if (si_gpu_branch_unit_read_latency < 0)
		fatal("%s: invalid value for 'ReadLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_read_buffer_size = config_read_int(gpu_config, section, "ReadBufferSize", si_gpu_branch_unit_read_buffer_size);
	if (si_gpu_branch_unit_read_buffer_size < 1)
		fatal("%s: invalid value for 'ReadBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_exec_latency = config_read_int(gpu_config, section, "ExecLatency", si_gpu_branch_unit_exec_latency);
	if (si_gpu_branch_unit_exec_latency < 0)
		fatal("%s: invalid value for 'ExecLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_exec_buffer_size= config_read_int(gpu_config, section, "ExecBufferSize", si_gpu_branch_unit_exec_buffer_size);
	if (si_gpu_branch_unit_exec_buffer_size < 1)
		fatal("%s: invalid value for 'ExecBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_write_latency = config_read_int(gpu_config, section, "WriteLatency", si_gpu_branch_unit_write_latency);
	if (si_gpu_branch_unit_write_latency < 0)
		fatal("%s: invalid value for 'WriteLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_branch_unit_write_buffer_size = config_read_int(gpu_config, section, "WriteBufferSize", si_gpu_branch_unit_write_buffer_size);
	...
	si_gpu_lds_width = config_read_int(gpu_config, section, "Width", si_gpu_lds_width);
	if (si_gpu_lds_width < 1)
		fatal("%s: invalid value for 'Width'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_issue_buffer_size = config_read_int(gpu_config, section, "IssueBufferSize", si_gpu_lds_issue_buffer_size);
	if (si_gpu_lds_issue_buffer_size < 1)
		fatal("%s: invalid value for 'IssueBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_decode_latency = config_read_int(gpu_config, section, "DecodeLatency", si_gpu_lds_decode_latency);
	if (si_gpu_lds_decode_latency < 0)
		fatal("%s: invalid value for 'DecodeLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_decode_buffer_size = config_read_int(gpu_config, section, "DecodeBufferSize", si_gpu_lds_decode_buffer_size);
	if (si_gpu_lds_decode_buffer_size < 1)
		fatal("%s: invalid value for 'DecodeBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_read_latency = config_read_int(gpu_config, section, "ReadLatency", si_gpu_lds_read_latency);
	if (si_gpu_lds_read_latency < 0)
		fatal("%s: invalid value for 'ReadLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_read_buffer_size = config_read_int(gpu_config, section, "ReadBufferSize", si_gpu_lds_read_buffer_size);
	if (si_gpu_lds_read_buffer_size < 1)
		fatal("%s: invalid value for 'ReadBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_max_inflight_mem_accesses = config_read_int(gpu_config, section, "MaxInflightMem", si_gpu_lds_max_inflight_mem_accesses);
	if (si_gpu_lds_max_inflight_mem_accesses < 1)
		fatal("%s: invalid value for 'MaxInflightMem'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_write_latency = config_read_int(gpu_config, section, "WriteLatency", si_gpu_lds_write_latency);
	if (si_gpu_lds_write_latency < 0)
		fatal("%s: invalid value for 'WriteLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_lds_write_buffer_size = config_read_int(gpu_config, section, "WriteBufferSize", si_gpu_lds_write_buffer_size);
	...
	si_gpu_vector_mem_width = config_read_int(gpu_config, section, "Width", si_gpu_vector_mem_width);
	if (si_gpu_vector_mem_width < 1)
		fatal("%s: invalid value for 'Width'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_issue_buffer_size = config_read_int(gpu_config, section, "IssueBufferSize", si_gpu_vector_mem_issue_buffer_size);
	if (si_gpu_vector_mem_issue_buffer_size < 1)
		fatal("%s: invalid value for 'IssueBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_decode_latency = config_read_int(gpu_config, section, "DecodeLatency", si_gpu_vector_mem_decode_latency);
	if (si_gpu_vector_mem_decode_latency < 0)
		fatal("%s: invalid value for 'DecodeLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_decode_buffer_size = config_read_int(gpu_config, section, "DecodeBufferSize", si_gpu_vector_mem_decode_buffer_size);
	if (si_gpu_vector_mem_decode_buffer_size < 1)
		fatal("%s: invalid value for 'DecodeBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_read_latency = config_read_int(gpu_config, section, "ReadLatency", si_gpu_vector_mem_read_latency);
	if (si_gpu_vector_mem_read_latency < 0)
		fatal("%s: invalid value for 'ReadLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_read_buffer_size = config_read_int(gpu_config, section, "ReadBufferSize", si_gpu_vector_mem_read_buffer_size);
	if (si_gpu_vector_mem_read_buffer_size < 1)
		fatal("%s: invalid value for 'ReadBufferSize'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_max_inflight_mem_accesses = config_read_int(gpu_config, section, "MaxInflightMem", si_gpu_vector_mem_max_inflight_mem_accesses);
	if (si_gpu_vector_mem_max_inflight_mem_accesses < 1)
		fatal("%s: invalid value for 'MaxInflightMem'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_write_latency = config_read_int(gpu_config, section, "WriteLatency", si_gpu_vector_mem_write_latency);
	if (si_gpu_vector_mem_write_latency < 0)
		fatal("%s: invalid value for 'WriteLatency'.\n%s", si_gpu_config_file_name, err_note);

	si_gpu_vector_mem_write_buffer_size = config_read_int(gpu_config, section, "WriteBufferSize", si_gpu_vector_mem_write_buffer_size);
	...
	si_gpu_lds_size = config_read_int(gpu_config, section, "Size", si_gpu_lds_size);
	si_gpu_lds_alloc_size = config_read_int(gpu_config, section, "AllocSize", si_gpu_lds_alloc_size);
	si_gpu_lds_block_size = config_read_int(gpu_config, section, "BlockSize", si_gpu_lds_block_size);
	si_gpu_lds_latency = config_read_int(gpu_config, section, "Latency", si_gpu_lds_latency);
	si_gpu_lds_num_ports = config_read_int(gpu_config, section, "Ports", si_gpu_lds_num_ports);

	if ((si_gpu_lds_size & (si_gpu_lds_size - 1)) || si_gpu_lds_size < 4)
		fatal("%s: %s->Size must be a power of two and at least 4.\n%s", si_gpu_config_file_name, section, err_note);

	if (si_gpu_lds_alloc_size < 1)
		fatal("%s: invalid value for %s->Allocsize.\n%s", si_gpu_config_file_name, section, err_note);

	if (si_gpu_lds_size % si_gpu_lds_alloc_size)
		fatal("%s: %s->Size must be a multiple of %s->AllocSize.\n%s", si_gpu_config_file_name, section, section, err_note);
	...
	if (si_gpu_lds_size < si_gpu_lds_block_size)
		fatal("%s: %s->Size cannot be smaller than %s->BlockSize * %s->Banks.\n%s", si_gpu_config_file_name, section, section, section, err_note);
...
void si_gpu_dump_report(void)
{
	...
	inst_per_cycle = asTiming(si_gpu)->cycle ? (double)(asEmu(si_emu)->instructions/asTiming(si_gpu)->cycle) : 0.0;
	fprintf(f, "[ Device ]\n\n");
...
int SIGpuRun(Timing *self)
{
	...
	//star changes to 2M cycles.
	if ((asTiming(si_gpu)->cycle-gpu->last_complete_cycle) > 2000000)
	{
