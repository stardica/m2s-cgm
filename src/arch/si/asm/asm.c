...
#undef DEFINST
	...
	//star >> some notes:
	//All of the instructions are read in form the #include "asm.dat"
	//each of the instructions _name fields are enumerated and assigned a value from 1 to SI_INST_COUNT (i.e. total number of instructions).
	//each instruction is loaded into struct si_inst_info_t si_inst_info[SI_INST_COUNT];
	//going forward work is performed. this is just an init function.


	/* Tables of pointers to 'si_inst_info' */
	for (i = 1; i < SI_INST_COUNT; i++)
	{
		//don't use si_inst_info[0];
		//this is not initialized by the macro.
		info = &si_inst_info[i];
...
int si_inst_decode(void *buf, struct si_inst_t *inst, unsigned int offset)
{
	...
	else if (inst->micro_inst.mimg.enc == 0x3C)
	{
		...
		{
			fatal("Unimplemented Instruction: MIMG:%d  // %08X: %08X %08X\n", inst->micro_inst.mimg.op, offset, *(unsigned int *)buf, *(unsigned int *)(buf+4));
		}
