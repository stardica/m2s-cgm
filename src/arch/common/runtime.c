...
void runtime_register(char *name, char *lib_name, char *redirect_lib_name, int syscall_code, runtime_abi_func_t abi_func)
{
	//create pointer to runtime_t
	struct runtime_t *runtime;

	/* Create runtime */
	//star >> point to new runtime_t struct.
	...
	// Star >> the abi function is here... (opencl_abi_call)
