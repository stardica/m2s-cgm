		...
		arch_timing_done_func_t timing_done_func)
{
	...
	/* Check no duplicates */
	arch = arch_get(name);
	//star >> if not NULL quit and panic.
	if (arch)
		panic("%s: duplicated architecture", __FUNCTION__);

	/* Check list size */
	//checks if there are two many architectures
	if (arch_list_count == ARCH_LIST_MAX_SIZE)
		panic("%s: architecture list too big - increase ARCH_LIST_MAX_SIZE", __FUNCTION__);
...
void arch_init(void)
{
	...
	for (i = 0; i < arch_list_count; i++)
	{
		...
		if (arch->sim_kind == arch_sim_kind_detailed)
		{
			...
			// star >> calls the function by the pointer stored in the struct type arch_t. Here are the assignments:
			// x86_emu_init, x86_emu_done, x86CpuReadConfig designate pointers to call back functions

			// star >> for the x86Cpu
			// *arch_register(char *name, = x86
			// char *prefix, = x86
			// enum arch_sim_kind_t sim_kind, = x86_sim_kind
			// arch_emu_init_func_t emu_init_func, = x86_emu_init
			// arch_emu_done_func_t emu_done_func, = x86_emu_done
			// arch_timing_read_config_func_t timing_read_config_func, = x86CpuReadConfig
			// arch_timing_init_func_t timing_init_func, = Null
			// arch_timing_done_func_t timing_done_func) = null


			// star >> for the SIGpu
			// *arch_register(char *name = SouthernIslands ,
			// char *prefix = si,
			// enum arch_sim_kind_t sim_kind, = si_sim_kind
			// arch_emu_init_func_t emu_init_func, = si_emu_init
			// arch_emu_done_func_t emu_done_func, = si_emu_done
			// arch_timing_read_config_func_t timing_read_config_func, = si_gpu_read_config
			// arch_timing_init_func_t timing_init_func, = si_gpu_init
			// arch_timing_done_func_t timing_done_func) = si_gpu_done


			// x86_emu_init, x86_emu_done, x86CpuReadConfig
			// si_sim_kind, si_emu_init, si_emu_done, si_gpu_read_config, si_gpu_init, si_gpu_done

			if (arch->timing_read_config_func)
				//star >> calls... "X86CpuReadConfig(void)" for x86
				//star >> calls... "si_gpu_read_config(void) for SIGpu
				arch->timing_read_config_func();

		}

		/* Initialize emulator */
		// star >> calls the function by the pointer stored in the struct arch_t.
		if (arch->emu_init_func)
			//star >> calls... "x86_emu_init(void)" for x86
			//star >> calls... "si_emu_init(void)" for SIGpu
			arch->emu_init_func();
		...
		if (arch->sim_kind == arch_sim_kind_detailed)
		{
			...
			if (arch->timing_init_func)
				//star >> calls... "NULL" for x86. Note: this was hard coded by developers.
				//star >> calls... "si_gpu_init(void)" for SiGpu
...
void arch_for_each(arch_callback_func_t callback_func, void *user_data)
{
	...
	for (i = 0; i < arch_list_count; i++)
	{
		//star >> calls this function for each arch.
		//((arch_callback_func_t) arch_dump_summary, f)
	...
	*num_timing_active_ptr = 0;
	...
	for (i = 0; i < arch_list_count; i++)
	{
		...
		else
		{
			...
			//star >> checking these variables out.
			//printf("last TC= %lld\n", arch->last_timing_cycle);
			//printf("Cycle = %lld\n", cycle);
			//printf("Run = %d\n", run);
			//fflush(stdout);
