...
int X86ThreadLookupBranchPred(X86Thread *self, struct x86_uop_t *uop)
{
	...
	if (x86_bpred_kind == x86_bpred_kind_twolevel || x86_bpred_kind == x86_bpred_kind_comb)
	{
		...
		uop->twolevel_pred = bpred->twolevel_pht[uop->twolevel_pht_row * x86_bpred_twolevel_l2size + uop->twolevel_pht_col] > 1;
...
void X86ThreadUpdateBranchPred(X86Thread *self, struct x86_uop_t *uop)
{
	...
	if (x86_bpred_kind == x86_bpred_kind_bimod || (x86_bpred_kind == x86_bpred_kind_comb && !uop->choice_pred))
	{
	...
	if (x86_bpred_kind == x86_bpred_kind_twolevel || (x86_bpred_kind == x86_bpred_kind_comb && uop->choice_pred))
	{
		...
		*pbhr = ((*pbhr << 1) | taken) & (x86_bpred_twolevel_l2height - 1);
		...
		pctr = &bpred->twolevel_pht[uop->twolevel_pht_row * x86_bpred_twolevel_l2size + uop->twolevel_pht_col];
		*pctr = taken ? MIN(*pctr + 1, 3) : MAX(*pctr - 1, 0);
	}
...
void X86ReadBranchPredConfig(struct config_t *config)
{
	...
	//star >> the "<<" is a bitwise left shift
