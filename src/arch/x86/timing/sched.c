...
void X86ThreadSchedule(X86Thread *self)
{
	...
	if (ctx)
	{
		...
		if (!ctx->evict_signal && asTiming(cpu)->cycle >= ctx->alloc_cycle + x86_cpu_context_quantum)
		{
			int found = 0;
...
void X86CpuAllocateContext(X86Cpu *self, X86Context *ctx)
{
	...
	/* Trace */
	x86_trace("x86.map_ctx ctx=%d core=%d thread=%d ppid=%d\n", ctx->pid, core_index, thread_index, ctx->parent ? ctx->parent->pid : 0);
}
...
void X86CpuMapContext(X86Cpu *self, X86Context *ctx)
{
	 ...
	 * the one with the smaller number of contexts mapped. */
	min_core = -1;
	min_thread = -1;
	node = 0;
	...
	DOUBLE_LINKED_LIST_INSERT_TAIL(self->cores[core]->threads[thread], mapped, ctx);
...
void X86CpuSchedule(X86Cpu *self)
{
	...
	quantum_expired = asTiming(self)->cycle >= self->min_alloc_cycle + x86_cpu_context_quantum;
