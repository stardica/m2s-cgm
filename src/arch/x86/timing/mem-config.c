...
void X86CpuMemConfigParseEntry(Timing *self, struct config_t *config, char *section)
{
	...
	if (core_index < 0)
		fatal("%s: section [%s]: invalid or missing value for 'Core'", file_name, section);

	/* Read thread */
	thread_index = config_read_int(config, section, "Thread", -1);
	if (thread_index < 0)
		fatal("%s: section [%s]: invalid or missing value for 'Thread'", file_name, section);
	...
	{
		data_module_name = inst_module_name = config_read_string(config, section, "Module", NULL);
		assert(data_module_name);
	}
