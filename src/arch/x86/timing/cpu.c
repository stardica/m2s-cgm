...
void X86CpuReadConfig(void)
{
	...
	//printf("*********core number %d", x86_cpu_num_cores);
	//printf("*********thread number %d", x86_cpu_num_threads);
...
{
	//star >> notice cpu, then core, then thread.
	...
	fprintf(f, "IPCLastDump = %.4g   ; IPC since last dump\n", asTiming(cpu)->cycle - cpu->last_dump > 0 ? (double) (cpu->num_committed_uinst - cpu->last_committed) / (asTiming(cpu)->cycle - cpu->last_dump) : 0);
	fprintf(f, "\n");
...
void X86CpuDumpSummary(Timing *self, FILE *f)
{
	...
	inst_per_cycle = asTiming(cpu)->cycle ? (double) cpu->num_committed_inst / asTiming(cpu)->cycle : 0.0;
	uinst_per_cycle = asTiming(cpu)->cycle ? (double) cpu->num_committed_uinst / asTiming(cpu)->cycle : 0.0;
...
#define DUMP_CORE_STRUCT_STATS(NAME, ITEM) 		{ 																										\
												...
												fprintf(f, #NAME ".Occupancy = %.2f\n", asTiming(self)->cycle ? (double) core->ITEM##_occupancy / asTiming(self)->cycle : 0.0);	\
...
#define DUMP_THREAD_STRUCT_STATS(NAME, ITEM) 	{ 																										\
												fprintf(f, #NAME ".Size = %d\n", (int) x86_##ITEM##_size); 												\
												if (x86_cpu_occupancy_stats) 																			\
												fprintf(f, #NAME ".Occupancy = %.2f\n", asTiming(self)->cycle ? (double) thread->ITEM##_occupancy / asTiming(self)->cycle : 0.0); \
												fprintf(f, #NAME ".Full = %lld\n", thread->ITEM##_full); 												\
												fprintf(f, #NAME ".Reads = %lld\n", thread->ITEM##_reads); 												\
												fprintf(f, #NAME ".Writes = %lld\n", thread->ITEM##_writes); 											\
												}
...
void X86CpuDumpReport(X86Cpu *self, FILE *f)
{
	...
	X86CpuDumpUopReport(self, f, self->num_dispatched_uinst_array, "Dispatch", x86_cpu_dispatch_width);
	...
	fprintf(f, "Commit.PredAcc = %.4g\n", self->num_branch_uinst ? (double) (self->num_branch_uinst - self->num_mispred_branch_uinst) / self->num_branch_uinst : 0.0);
	fprintf(f, "\n");
	...
	for (i = 0; i < x86_cpu_num_cores; i++)
	{
		...
		fprintf(f, "; Dispatch stage\n");
		//X86CpuDumpUopReport(self, f, self->num_dispatched_uinst_array, "Dispatch", x86_cpu_dispatch_width);
		X86CpuDumpUopReport(self, f, core->num_dispatched_uinst_array, "Dispatch", x86_cpu_dispatch_width);
		...
		/* Commit stage */
		fprintf(f, "; Commit stage\n");
		X86CpuDumpUopReport(self, f, core->num_committed_uinst_array, "Commit", x86_cpu_commit_width);
		...
		fprintf(f, "Commit.PredAcc = %.4g\n", core->num_branch_uinst ? (double) (core->num_branch_uinst - core->num_mispred_branch_uinst) / core->num_branch_uinst : 0.0);
		fprintf(f, "\n");
		...
		for (j = 0; j < x86_cpu_num_threads; j++)
		{
			...
			/* Dispatch stage */
			fprintf(f, "; Dispatch stage\n");
			X86CpuDumpUopReport(self, f, thread->num_dispatched_uinst_array, "Dispatch", x86_cpu_dispatch_width);

			/* Issue stage */
			fprintf(f, "; Issue stage\n");
			X86CpuDumpUopReport(self, f, thread->num_issued_uinst_array, "Issue", x86_cpu_issue_width);

			/* Commit stage */
			fprintf(f, "; Commit stage\n");
			X86CpuDumpUopReport(self, f, thread->num_committed_uinst_array, "Commit", x86_cpu_commit_width);
			...
			fprintf(f, "Commit.PredAcc = %.4g\n", thread->num_branch_uinst ? (double) (thread->num_branch_uinst - thread->num_mispred_branch_uinst) / thread->num_branch_uinst : 0.0);
			fprintf(f, "\n");
...
void X86CpuDumpUopReport(X86Cpu *self, FILE *f, long long *uop_stats, char *prefix, int peak_ipc)
{
	...
	fprintf(f, "%s.IPC = %.4g\n", prefix, asTiming(self)->cycle ? (double) uinst_total / asTiming(self)->cycle : 0.0);
	fprintf(f, "%s.DutyCycle = %.4g\n", prefix, asTiming(self)->cycle && peak_ipc ? (double) uinst_total / asTiming(self)->cycle / peak_ipc : 0.0);
	fprintf(f, "\n");
}
...
int X86CpuRun(Timing *self){
	...
	if (x86_emu_max_inst && cpu->num_committed_inst >= x86_emu_max_inst - x86_cpu_fast_forward_count)
