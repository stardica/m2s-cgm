...
void X86ThreadInitTraceCache(X86Thread *self)
{
	...
	/* Initialization */
	snprintf(name, sizeof name,"Core[%d].Thread[%d].TraceCache", core->id, self->id_in_core);
...
void X86ThreadDumpTraceCacheReport(X86Thread *self, FILE *f)
{
	...
	fprintf(f, "TraceCache.HitRatio = %.4g\n", trace_cache->accesses ? (double) trace_cache->hits / trace_cache->accesses : 0.0);
	...
	fprintf(f, "TraceCache.TraceLength = %.2f\n", trace_cache->trace_length_count ? (double) trace_cache->trace_length_acc / trace_cache->trace_length_count : 0);
	fprintf(f, "\n");
}
...
struct x86_trace_cache_t *x86_trace_cache_create(char *name)
{
	...
	trace_cache->entry = xcalloc(x86_trace_cache_num_sets * x86_trace_cache_assoc, X86_TRACE_CACHE_ENTRY_SIZE);
...
void X86ReadTraceCacheConfig(struct config_t *config)
{
	...
	if ((x86_trace_cache_assoc & (x86_trace_cache_assoc - 1)) || !x86_trace_cache_assoc)
		fatal("%s: %s: 'Assoc' must be a power of 2 greater than 0", file_name, section);
	...
	if (x86_trace_cache_branch_max > x86_trace_cache_trace_size)
		fatal("%s: %s: 'BranchMax' must be equal or less than 'TraceSize'", file_name, section);
