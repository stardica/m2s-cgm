...
void X86ThreadInitRegFile(X86Thread *self)
{
	...
	/* Create it */
	self->reg_file = reg_file = x86_reg_file_create(x86_reg_file_int_local_size, x86_reg_file_fp_local_size, x86_reg_file_xmm_local_size);
...
void X86ThreadDumpRegFile(X86Thread *self, FILE *f)
{
	...
	{
		fprintf(f, "  %3d%c[%d-%d]", i, self->reg_file->int_phreg[i].busy ? ' ' : '*', self->reg_file->int_phreg[i].busy, self->reg_file->int_phreg[i].pending);
	...
	{
		fprintf(f, "  %3d%c[%d-%d]", i, self->reg_file->fp_phreg[i].busy ? ' ' : '*', self->reg_file->fp_phreg[i].busy, self->reg_file->fp_phreg[i].pending);
	...
	{
		fprintf(f, "  %3d%c[%d-%d]", i, self->reg_file->xmm_phreg[i].busy ? ' ' : '*', self->reg_file->xmm_phreg[i].busy, self->reg_file->xmm_phreg[i].pending);
