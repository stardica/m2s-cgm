...
static void X86ContextUpdateState(X86Context *self, X86ContextState state)
{
	...
	/* Remove contexts from the following lists: running, suspended, zombie */
	...
	if (!(self->state & X86ContextSuspended) && !(self->state & X86ContextFinished) && !(self->state & X86ContextZombie) && !(self->state & X86ContextLocked))
	...
	/* Start/stop x86 timer depending on whether there are any contexts currently running. */
...
/* If the context is running a 'x86_emu_host_thread_suspend' thread, cancel it and schedule call to 'x86_emu_process_events' */
...
int X86ContextFutexWake(X86Context *self, unsigned int futex, unsigned int count, unsigned int bitset)
{
	X86Emu *emu = self->emu;
...
void *X86ContextHostThreadTimer(void *arg)
{
	...
	printf("X86ContextHostThreadTimer spawned\n");
	fflush(stdout);
