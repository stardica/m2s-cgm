...
void X86ContextSyscall(X86Context *self)
{
	...
	if (code < 1 || code >= x86_sys_code_count)
	{
		...
		/*printf("syscall ecx %u edx %u esi %u\n", regs->ecx, regs->edx, regs->esi);*/
...
static int x86_sys_setitimer_impl(X86Context *ctx)
{
	...
	ctx->itimer_value[which] = now + itimerval.it_value.tv_sec * 1000000 + itimerval.it_value.tv_usec;
	ctx->itimer_interval[which] = itimerval.it_interval.tv_sec * 1000000 + itimerval.it_interval.tv_usec;
...
static int x86_sys_clone_impl(X86Context *ctx)
{
	...
	{
		str_map_flags(&sys_clone_flags_map, flags & ~sys_clone_supported_flags, flags_str, MAX_STRING_SIZE);
		fatal("%s: not supported flags: %s\n%s", __FUNCTION__, flags_str, err_x86_sys_note);
	}
...
static void sim_fd_set_write(X86Context *ctx, unsigned int addr, fd_set *fds, int n)
{
	struct mem_t *mem = ctx->mem;
...
static int x86_sys_select_impl(X86Context *ctx)
{
	...
	if (!sim_fd_set_read(ctx, inp, &in, n) || !sim_fd_set_read(ctx, outp, &out, n) || !sim_fd_set_read(ctx, exp, &ex, n))
	{
		return -EBADF;
	}
...
static int x86_sys_mremap_impl(X86Context *ctx)
{
	...
	if (new_len > old_len && mem_map_space(mem, addr + old_len, new_len - old_len) == addr + old_len)
	{
		mem_map(mem, addr + old_len, new_len - old_len, mem_access_read | mem_access_write);
		return addr;
	}
...
static int x86_sys_rt_sigaction_impl(X86Context *ctx)
{
	...
	if (old_act_ptr)
		mem_write(mem, old_act_ptr, sizeof(struct x86_sigaction_t), &ctx->signal_handler_table->sigaction[sig - 1]);
...
static int x86_sys_getrlimit_impl(X86Context *ctx)
{
	...
	default:
		fatal("%s: not implemented for res = %s.\n%s", __FUNCTION__, res_str, err_x86_sys_note);
	}
