...
void X86EmuDump(Object *self, FILE *f)
{
	...
	//star >> for loop that runs through the list and preforms the ContextDump.
...
{
	//star >> asX86Emu is just passing a pointer around.
	X86Emu *emu = asX86Emu(self);
...
void X86EmuProcessEvents(X86Emu *self)
{
	...
	for (ctx = self->suspended_list_head; ctx; ctx = next)
	{
		...
		if (X86ContextGetState(ctx, X86ContextCallback))
		{
			...
			if (ctx->can_wakeup_callback_func(ctx, ctx->can_wakeup_callback_data))
			{
				...
				//star >> testing entry here.
				//printf("Calling wake up function\n");
				//fflush(stdout);
	...
	for (ctx = self->context_list_head; ctx; ctx = ctx->context_list_next)
	{
		...
		//printf("x86EmuProcessEvents Loop 2\n");
		//fflush(stdout);
	...
	{
		//printf("x86EmuProcessEvents Loop 3\n");
		//fflush(stdout);
...
{
	//star >> x86Context & X86Emu are typedef'd
	X86Context *ctx;
	struct x86_loader_t *loader;
...
void x86_emu_init(void){
	...
	/* Endian check */
	//star >> not complicated, compares an input value to ensure that the "host" system is little endian.
	//Intel CPUs are little endian. This is a neat trick.
	union
	{
	...
	//end endian check, so i suppose this wouldn't run on some older Macs

	/* Host types */
	//star >>checking to make sure that the two expressions "long long" and 8 (bytes) are equal
	// if they are not emulator quits.
	M2S_HOST_GUEST_MATCH(sizeof(long long), 8);
	M2S_HOST_GUEST_MATCH(sizeof(int), 4);
	M2S_HOST_GUEST_MATCH(sizeof(short), 2);

	//star >> checks passed, moves on to start emulator.
	/* Create x86 emulator */

	//call backs: x86EmuCreate
	x86_emu = new(X86Emu);

	/* Initialize */
	//call backs: X86AsmCreate
	x86_asm_init();
	x86_uinst_init();

//star >> comented this out.
/*
#ifdef HAVE_OPENGL
	 GLUT
	glut_init();
...
*/
	...
	//star >> this is empty and does nothing...
