...
void X86ContextError(X86Context *self, char *fmt, ...)
{
	...
	/* Error */
	fprintf(stderr, "fatal: x86 context %d at 0x%08x inst %lld: ", self->pid, self->curr_eip, asEmu(emu)->instructions);
...
void X86ContextExecuteInst(X86Context *self)
{
	...
	if (debug_status(x86_context_isa_debug_category))
	{
		X86ContextDebugISA("%d %8lld %x: ", self->pid, asEmu(emu)->instructions, self->curr_eip);
