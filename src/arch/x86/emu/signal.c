...
{
	fprintf(f, "handler=0x%x, flags=0x%x, restorer=0x%x, mask=0x%llx", sim->handler, sim->flags, sim->restorer, sim->mask);
}
...
void X86ContextCheckSignalHandlerIntr(X86Context *ctx)
{
	...
	for (sig = 1; sig <= 64; sig++)
		if (x86_sigset_member(&ctx->signal_mask_table->pending, sig) && !x86_sigset_member(&ctx->signal_mask_table->blocked, sig))
			break;
	assert(sig <= 64);

	/* If signal handling uses 'SA_RESTART' flag, set return address to system call. */
