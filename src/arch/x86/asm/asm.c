...
static void x86_asm_inst_info_insert_at(struct x86_inst_info_elem_t **table, struct x86_inst_info_elem_t *elem, int at)
{
...
void x86_asm_init(void)
{
	...
	//star >> runs class new and X86AsmCreate
	x86_asm = new(X86Asm);
}
...
void x86_asm_disassemble_binary(char *path)
{
	...
	//star >> this provides access to the ELF data input by the user.
