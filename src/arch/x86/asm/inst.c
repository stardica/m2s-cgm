...
void x86_inst_dump_buf(struct x86_inst_t *inst, char *buf, int size)
{
	...
	while (*fmt)
	{
		...
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->modrm_reg + x86_inst_reg_al));
		}
		else if (asm_is_token(fmt, "r16", &length))
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->modrm_reg + x86_inst_reg_ax));
		}
		else if (asm_is_token(fmt, "r32", &length))
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->modrm_reg + x86_inst_reg_eax));
		}
		else if (asm_is_token(fmt, "rm8", &length))
		{
			if (inst->modrm_mod == 0x03)
				str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->modrm_rm + x86_inst_reg_al));
			else
			{
		...
		{
			if (inst->modrm_mod == 3)
				str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->modrm_rm + x86_inst_reg_eax));
			else
			{
				str_printf(&buf, &size, "WORD PTR ");
				x86_inst_addr_dump_buf(inst, &buf, &size);
		...
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->opindex + x86_inst_reg_al));
		}
		else if (asm_is_token(fmt, "ir16", &length))
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->opindex + x86_inst_reg_ax));
		}
		else if (asm_is_token(fmt, "ir32", &length))
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->opindex + x86_inst_reg_eax));
		}
		else if (asm_is_token(fmt, "sreg", &length))
		{
			str_printf(&buf, &size, "%s", str_map_value(&x86_inst_reg_map, inst->reg + x86_inst_reg_es));
		}
...
void x86_inst_decode(struct x86_inst_t *inst, unsigned int eip, void *buf)
{
	...
	{
		info = elem->info;
		if (info->nomatch_mask && (buf32 & info->nomatch_mask) == info->nomatch_result)
			continue;
		if ((buf32 & info->match_mask) == info->match_result && info->prefixes == inst->prefixes)
			break;
	}
	...
	if (inst->modrm_size)
	{
		...
		/* Access ModRM table */
		modrm_table_entry = &x86_inst_modrm_table[(inst->modrm_mod << 3) | inst->modrm_rm];
