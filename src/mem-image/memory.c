 ...
 * mem. The returned buffer is null if addr+size exceeds the page boundaries. */
...
static void mem_access_page_boundary(struct mem_t *mem, unsigned int addr, int size, void *buf, enum mem_access_t access)
{
	...
	if (!page)
	{
		...
		{
			page = mem_page_create(mem, addr, mem_access_read | mem_access_write | mem_access_exec | mem_access_init);
		}
	}
	assert(page);
