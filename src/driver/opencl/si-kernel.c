...
struct opencl_si_kernel_t *opencl_si_kernel_create(struct opencl_si_program_t *program, char *name)
{
	struct opencl_si_kernel_t *kernel;
...
void opencl_si_kernel_setup_ndrange_constant_buffers(struct si_ndrange_t *ndrange)
{
	struct si_buffer_desc_t buffer_desc;
	...
	opencl_si_create_buffer_desc(ndrange->cb0, SI_EMU_CONST_BUF_0_SIZE, 1, si_arg_i32, &buffer_desc);

	si_ndrange_insert_buffer_into_const_buf_table(ndrange, &buffer_desc, 0);

	opencl_si_create_buffer_desc(ndrange->cb1, SI_EMU_CONST_BUF_1_SIZE, 1, si_arg_i32, &buffer_desc);
...
void opencl_si_kernel_setup_ndrange_args(struct opencl_si_kernel_t *kernel, struct si_ndrange_t *ndrange)
{
	struct si_arg_t *arg;
...
void opencl_si_kernel_debug_ndrange_state(struct opencl_si_kernel_t *kernel, struct si_ndrange_t *ndrange)
{
        int i = 0;
