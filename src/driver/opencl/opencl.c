...
int opencl_abi_call(X86Context *ctx)
{
	...
	if (code <= opencl_abi_invalid || code >= opencl_abi_call_count)
		fatal("%s: invalid OpenCL ABI call (code %d).\n%s", __FUNCTION__, code, opencl_err_abi_call);
...
static int opencl_abi_si_ndrange_send_work_groups_can_wakeup(X86Context *ctx, void *user_data)
{
	assert(!user_data);
...
{
	if (driver_state.wait_for_ndrange_completion && !list_count(si_emu->running_work_groups) && !list_count(si_emu->waiting_work_groups))
	{
