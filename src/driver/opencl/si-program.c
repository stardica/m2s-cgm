...
struct opencl_si_constant_buffer_t *opencl_si_constant_buffer_create(int id, unsigned int device_ptr, unsigned int size){
...
static void opencl_si_program_initialize_constant_buffers(struct opencl_si_program_t *program)
{
	...
	for (i = 2; i < 25; i++) 
	{
		...
		mem_write(si_emu->video_mem, si_emu->video_mem_top, symbol->size, elf_buffer.ptr);

		/* Create buffer */
		constant_buffer = opencl_si_constant_buffer_create(i, si_emu->video_mem_top, symbol->size);
...
void opencl_si_program_set_binary(struct opencl_si_program_t *program, void *buf, unsigned int size){
