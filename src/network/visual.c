...
struct net_graph_t *net_visual_calc(struct net_t *net)
{
	...
	for(i = 0; i < list_count(net->link_list); i++)
	{
		...
		int j = 0;
		for (j = 0; j < list_count(graph->vertex_list); j++)
		{
			struct net_graph_vertex_t *vertex;
		...
		for (j = 0; j < list_count(graph->edge_list); j++)
		{
	...
	for (i = 0; i < list_count(graph->vertex_list); i++)
	{
		...
		if (vertex->kind == net_vertex_bus)
		{
			...
			int j = 0;
			for (j = 0; j < list_count(bus_node->src_buffer_list); j++ )
			{
			...
			for (j = 0; j < list_count(bus_node->dst_buffer_list); j++ )
			{
				int check = 0;
				...
				int l = 0;
				for (l = 0; l < list_count(graph->edge_list); l++)
				{
					struct net_graph_edge_t *temp_edge;
	...
	while (label <= graph->vertex_count)
	{
		...
		for (i = 0; i < list_count(graph->vertex_list); i++)
		{
			...
			if (vertex->key_val == 0)
			{
				...
				int checker = 0;
				for (j = 0; j < list_count(vertex->incoming_vertex_list); j++)
				{
		...
		if (list_count(candidate_list) != 0)
		{
			...
			assert(candidate);
			for (i = 1; i < list_count(candidate_list); i++)
			{
...
void net_graph_coffman_graham_layering (struct net_graph_t *graph, int width)
{
	...
	while (list_count(V) != 0)
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			int layer_count = 0;
			for (j = 0; j < list_count(candidate->outgoint_vertex_list); j++)
			{
...
void net_graph_cross_reduction(struct net_graph_t *graph, int layer_count)
{
	...
	for (x = layer_count; x >= 0 ; x--)
	{
		...
		for(i = list_count(key_list) - 1; i >= 0; i--)
		{
			struct net_graph_vertex_t *vertex;
