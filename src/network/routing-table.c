...
struct net_routing_table_entry_t *net_routing_table_lookup(struct net_routing_table_t *routing_table, struct net_node_t *src_node, struct net_node_t *dst_node)
{
	struct net_routing_table_entry_t *entry;
	...
	struct net_node_t *next_node, int vc_num)
{
	...
	for (k = 0; (k < list_count(src_node->output_buffer_list) && route_check != 1); k++)
	{
		...
		else if (buffer->kind == net_buffer_bus)
		{
			...
			int i = 0;
			for (i = 0; i < list_count(bus_node->dst_buffer_list); i++)
			{
				struct net_buffer_t *dst_buffer;
