...
static void net_traffic_uniform(struct net_t *net, double *inject_time)
{
	while (1)
	{
		...
		for (i = 0; i < net->node_count; i++)
		{
			...
			{
				assert(0);
				dst_node = list_get(net->node_list, random() % list_count(net->node_list));
...
void net_init(void)
{
	...
	if (*net_visual_file_name)
	{
		...
		if (!net_visual_file)
			fatal("%s: cannot write on network visualization file", net_visual_file_name);
	}
}
