...
struct esim_domain_t *esim_domain_create(int frequency)
{
	...
	//star >> convert from 4000 (4.0 Giga hrtz) to cycles per second
	domain->cycle_time = 1000000ll / frequency;  /* Picoseconds */
	//printf("********cycle time: %lld***********", domain->cycle_time);
...
struct esim_event_info_t *esim_event_info_create(int id, char *name, esim_event_handler_t handler, struct esim_domain_t *domain)
{
...
static int esim_drain_heap(void)
{
	...
	while (1)
	{
		...
		{
			esim_dump(stderr, 20);
			warning("%s: number of finalization events exceeds %d - stopped.\n%s", __FUNCTION__, ESIM_MAX_FINALIZATION_EVENTS, esim_err_finalization);
			return 1;
		}
	}
...
//	star >> looking at how the handler is loaded.
// 	esim_register_event_with_name(mod_handler_nmoesi_load, mem_domain_index, "mod_nmoesi_load");
//returns the position of the function in the esim_event_info_list
int esim_register_event_with_name(esim_event_handler_t handler, int domain_index, char *name)
{
	...
	/* Get frequency domain. Allow invalid domains for event 'Invalid' and 'None'. */
	domain = list_get(esim_domain_list, domain_index);
...
void esim_schedule_event(int event_index, void *data, int cycles)
{
	...
	if (!esim_overload_shown && esim_event_heap->count >= ESIM_OVERLOAD_EVENTS)
	{
		...
		warning("%s: number of in-flight events exceeds %d.\n%s", __FUNCTION__, ESIM_OVERLOAD_EVENTS, esim_err_overload);
	}
}
...
void esim_process_events(int forward)
{
	...
	// star >> processes all of the events scheduled
