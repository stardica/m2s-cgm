...
//star pointer to class_t struct
struct class_t *class_list_head;
struct class_t *class_list_tail;


//star >> this is run from the class register macro
...
void class_init(void)
{
	...
	/* Already initialized */
	//checks t see if object class exists
	if (ObjectClass.id)
		return;
	...
	//star >> casts the pointers to unsigned int and gets the difference.
	//Object "o" is thrown away on return.
	ObjectClass.info_offset = (unsigned int) ((void *) &o.__info - (void *) &o);
	assert(ObjectClass.info_offset == 0);
	ObjectClass.parent = NULL;
	//star >> this sets this element to point to the ObjectDestroy a function.
	//this is empty
	...
	//star >> points the class_list_head and tail to the new object class.
...
void class_register(struct class_t *c)
{
	...
	//star >> checks x86EmuClass.id
	if (c->id)
		panic("%s: class '%s' registered twice", __FUNCTION__, c->name);
	

	/* Compute class ID */
	assert(c->name && *c->name);

	//assigns the class ID via hashing.
	...
	//star >> this inserts the class into the list of classes.
	//star >> has class list tail pointer and next pointer point to this class.
...
void *class_new(struct class_t *c)
{
	...
	do
	{
		...
		if (!c->id)
			panic("%s: class has not been registered - use CLASS_REGISTER()", __FUNCTION__);

		/* Get parent class info */
		parent_info = c->parent ? (struct class_info_t *) (p + c->parent->info_offset): NULL;
