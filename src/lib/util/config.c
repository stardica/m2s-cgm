...
static void get_section_var_from_item(char *item, char *section, int section_size, char *var, int var_size)
{
	char *section_brk;
...
static int get_var_value_from_item(char *item, char *var, int var_size, char *value, int value_size)
{
...
void config_free(struct config_t *config)
{
	...
	{
		get_section_var_from_item(item, section, sizeof section, var, sizeof var);
		if (var[0])
		{
			free(value);
			continue;
...
void config_load(struct config_t *config)
{
	...
	{
		/* Read a line */
		//star >> if you didn't get anything go back to beginning of while.
		line_num++;
		...
		if (line_trim[0] == '[' && line_trim[length - 1] == ']')
		{
			...
			if (err)
				fatal("%s: line %d: duplicated section '%s'.\n%s", config->file_name, line_num, section, config_err_format);
		...
		if (!section[0])
			fatal("%s: line %d: section name expected.\n%s", config->file_name, line_num, config_err_format);
		...
		if (err)
			fatal("%s: line %d: duplicated variable '%s'.\n%s", config->file_name, line_num, var, config_err_format);
	}
...
int config_read_bool(struct config_t *config, char *section, char *var, int def)
{
	...
	/* True */
	if (!strcasecmp(result, "t") || !strcasecmp(result, "True") || !strcasecmp(result, "On"))
		return 1;
	
	/* False */
	if (!strcasecmp(result, "f") || !strcasecmp(result, "False") || !strcasecmp(result, "Off"))
		return 0;
...
void config_check(struct config_t *config)
{
	...
	//star >> files item with a value, then fills with the next item. exits for when the next item is void or null.
	for (item = hash_table_find_first(config->allowed_items, &property); item; item = hash_table_find_next(config->allowed_items, &property))
	{
		
		/* If this is an allowed (not mandatory) item, continue */
		//star>> goes back to top if allowed vice mandatory
		...
		if (!item_is_present(config, section, NULL))
			fatal("%s: section [ %s ] missing", config->file_name, section);
		if (!item_is_present(config, section, var))
			fatal("%s: section [ %s ]: missing mandatory variable '%s'", config->file_name, section, var);
	}
		...
		item; item = hash_table_find_next(config->items, NULL))
	{
		...
		/* It is not, error */
		if (!var[0])
			fatal("%s: invalid section [ %s ]", config->file_name, section);
		else
			fatal("%s: section [ %s ]: invalid variable '%s'", config->file_name, section, var);
	}
